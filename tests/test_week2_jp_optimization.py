"""tests for src/retrieval/jp_optimization.py (internal ADR § 2 + § 7 順守、 17 test)"""
...
# === Dictionary integrity (internal ADR § 2「30+ 用語」) ===


def test_pmi_domain_terms_count_meets_requirement():
    """internal ADR § 2: 30+ canonical terms 要件 (本 PJ = 32 件 actual)。"""
    assert len(PMI_DOMAIN_TERMS) >= 30


def test_pmi_domain_terms_has_critical_canonicals():
    """internal ADR IntegrationType + lifecycle critical terms 同期 verify。"""
