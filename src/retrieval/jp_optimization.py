"""ja_JP morpheme analysis + PMI domain dictionary (internal ADR § 2 順守、 2026-05-14 起草)

module boundaries 越境境界線 3 段階 順守:
- 段階 1 OS-primitives: fugashi + unidic-lite (MIT、 Paul McCann academic-grade、 MeCab Cython wrapper、 internal ADR update audit fact)
...
PMI_DOMAIN_TERMS: dict[str, list[str]] = {
    # M&A 統合 type (internal ADR IntegrationType enum と同期)
    ...
    # ownership / 経営形態 (internal ADR culture_profile 軸と同期)
...
def get_tagger() -> Any:
    ...
    Raises:
        RuntimeError: fugashi or unidic-lite not installed (internal ADR update pin 違反)
    """
    ...
    try:
        ...
        raise RuntimeError(
            "fugashi not installed (internal ADR update audit fact、 internal ADR § 2)。 "
...
# ============================================================================
# Entity name normalize (internal ADR § 2、 surface form → canonical)
# ============================================================================
