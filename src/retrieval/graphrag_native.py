...
"""GraphRAG コア要素 自作実装 (internal ADR § 1 順守、 2026-05-14 起草)
...
# existing internal 14 Object Type ((internal config)/internal_kb/ontology/types.yaml inherit)
...
# T5 6 type extend (internal ADR 順守、 DEC/PAT 完全 inherit、 PMI/REF/OUT extend、 LIL 新規起草)
...
# link_types 6 enum 固定 (internal ADR § link_types mapping + internal link_types.yaml inherit、 拡張禁止)
...
# PII vault field 名 (systemPatterns.md import 禁止 list + internal ADR PII/Op 分離 inherit、 25 件 narrow)
...
# Ontology validate gate threshold (internal ADR § 1)
ONTOLOGY_GATE_THRESHOLD: float = 0.80

# relationship confidence threshold (internal ADR § 5 link_type 6 enum 強制 mapping)
RELATIONSHIP_CONFIDENCE_THRESHOLD: float = 0.80

# community summary char limit (internal ADR § 1 「200 字 max、 PII redact 強制」)
...
class Relationship(BaseModel):
    """entity pair → 6 link_types 強制 mapping (internal ADR link_types mapping rule 順守)"""
...
- 上記 6 enum 以外を出力した場合 literal block されます (拡張禁止、 internal ADR inherit)
...
絶対 rules:
- summary は 200 字以内 (UI surface 制約 + token cost、 internal ADR § 1 制約)
...
# ============================================================================
# PII boundary check (internal ADR § 1 「vault field detect 時 literal block + alert」)
# ============================================================================
...
# ============================================================================
# Ontology validate gate (internal ADR § 1 「適合 < 80% で block + user gate prompt」)
# ============================================================================
...
def extract_entities(text: str, llm: "LLMProvider") -> list[Entity]:
    ...
    if not clean:
        raise PIIBoundaryViolation(
            ...
            "raw value embedding 禁止 (internal ADR § 1)"
        )
...
# ============================================================================
# Community detection (NetworkX Leiden thin wrapper、 BSD-3、 internal ADR update audit fact)
# ============================================================================
...
class PIIBoundaryViolation(ValueError):
    """PII vault field detect 時 raise (internal ADR § 1 PII boundary check)"""
