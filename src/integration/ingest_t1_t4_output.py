"""T1-T4 → T5 ingestion (internal ADR mapping rule literal 適用、 single-direction PoC)
...
mapping rule (internal ADR § T5 input mapping):
...
# === mapping table SSoT (internal ADR) ===
T1_T4_T5_MAPPING = {
    "T1": {
...
# === T4 CockpitProject → PMICase (internal ADR 一次連携) ===
def ingest_t4_cockpit_to_pmi(
    ...
    cp_dict expected fields (T4 ADR-303 schema、 既 redact 済 literal 前提):
      - cp_id: "CP-XXXXXX"
...
# === T4 DriverInsight + NextAction → Decision + Outcome (internal ADR) ===
def ingest_t4_driver_next_action(
    ...
    driver_insight expected (T4 ADR-303):
      - dr_id: "DR-XXXXXX"
...
# === T3 JPDay1Pattern → Pattern source (internal ADR 二次連携) ===
...
# === T1 ProfileOp + CompanyOp pair → PMICase.source_t1_pair (internal ADR 四次連携) ===
...
# === T2 DDP → PMICase.source_t2_ddp_id mapping (internal ADR 三次連携) ===
