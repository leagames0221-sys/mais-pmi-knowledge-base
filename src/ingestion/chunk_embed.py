"""chunk + embed pipeline (internal ADR § 2 順守、 LlamaIndex SentenceSplitter pattern 自作 + sentence-transformers + faiss thin wrapper)
...
def chunk_paper(
    ...
    if chunk_size <= overlap:
        raise ValueError(
            f"chunk_size ({chunk_size}) must be > overlap ({overlap})、 infinite loop 防御 (internal ADR § 2)"
        )
    if not markdown:
        return []
...
def embed_chunks(
    ...
    try:
        ...
        raise RuntimeError(
            "sentence-transformers not installed (cross-PJ python-ml-stack Standard Pin、 internal ADR § 2)。 "
...
class PaperIndex:
    """faiss IndexFlatIP thin wrapper + chunk_id reverse lookup (internal ADR § 2 段階 1 + 段階 2)。
    ...
    def _build_index(self) -> Any:
        try:
            ...
            raise RuntimeError(
                "faiss-cpu not installed (cross-PJ python-ml-stack、 internal ADR § 2)。 "
