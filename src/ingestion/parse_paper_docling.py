"""PDF paper parse via docling (internal ADR § 2 順守、 2026-05-14 起草)
...
def parse_paper_docling(pdf_path: str) -> str:
    ...
    Raises:
        RuntimeError: docling not installed (internal ADR update pin 違反)
    ...
    try:
        ...
        raise RuntimeError(
            "docling not installed (internal ADR update audit fact、 internal ADR § 2)。 "
    ...
    if not clean:
        raise ValueError(
            ...
            "raw value embedding 禁止 (internal ADR § 2、 paper_signatory + acknowledgments_raw 等の vault layer 分離必須)"
        )
    return markdown
