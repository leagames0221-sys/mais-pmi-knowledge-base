"""Citation infra (internal ADR § 3 順守、 2026-05-14 起草)
...
class CitationEngine:
    """PaperIndex 経由 citation generation (internal ADR § 3 PoC primary path)。
...
def build_llamaindex_citation_engine(
    ...
    Raises:
        RuntimeError: llama-index-core not installed (internal ADR update pin 違反)
    """
    try:
        # llama-index-core lazy import (internal ADR § 3 inherit pattern)
        import llama_index.core  # type: ignore[import-not-found]  # noqa: F401
    except ImportError as exc:
        raise RuntimeError(
            "llama-index-core not installed (internal ADR update audit fact、 internal ADR § 3)。 "
...
def format_citations(items: Sequence[RecommendationItem]) -> str:
    """Assistant RecommendationItem list の citation_array を「[REF-000001, PMI-000000007]」 format で surface (internal ADR § 3)。
...
def anthropic_citations_adapter_stub(query: str, response: CitationResponse) -> dict[str, Any]:
    """Anthropic Citations API (2026 official) swap path stub (internal ADR § 6 連携)。
    ...
    return {
        "model_hint": "claude-haiku-4-5 (internal ADR § 6 swap path)",
        "messages": [
            {
                "role": "user",
                "content": [
