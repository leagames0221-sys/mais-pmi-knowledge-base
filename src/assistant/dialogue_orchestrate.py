"""Assistant 型対話 full active (internal ADR § 1 順守、 2026-05-14 起草)

module boundaries 越境境界線 3 段階 順守:
- 段階 1 OS-primitives: anthropic SDK 経由 LLMProvider (literal use OK)
- 段階 2 business logic: listwise CoT prompt + AssistantCounter persistent + audit log emit + PII vault separation (literal 自作)
- 段階 3: 該当なし

internal ADR § 4 node_assistant_recommend_placeholder の Week 3 active 化 supersede:
...
# ============================================================================
# AssistantDialogueRequest schema (internal ADR § 1)
# ============================================================================
...
# ============================================================================
# LISTWISE CoT prompt (internal ADR § 1 自作、 OSS 全体 inject 不発)
# ============================================================================
...
# ============================================================================
# AssistantCounter (internal ADR § 4 persistent counter base、 jsonl tail + atomic write)
# ============================================================================


class AssistantCounter:
    """LIL id counter persistent storage (internal ADR § 4 順守)。
...
# ============================================================================
# Assistant recommend (LLM listwise CoT、 internal ADR § 1)
# ============================================================================
...
def assistant_recommend(
    ...
    """LLM listwise CoT で ranked recommendation 生成 (internal ADR § 1 順守)。

    Args:
        request: AssistantDialogueRequest (query + user_role + max_recommendations)
        top_k_cases: 多軸 weighted similarity top-K (internal ADR § 3 連携)
        community_summaries: Leiden community summarization (internal ADR § 1 連携)
        retrieved_papers: paper REF-id list (internal ADR § 2 連携、 Week 3 ingestion で literal active)
...
# ============================================================================
# Audit log emit (internal ADR § 4、 3 file 分離: assistant_log / assistant_counter / assistant_vault)
# ============================================================================
...
def emit_assistant_audit(
    ...
    """AssistantQuery schema literal emit + 3 file 分離 audit (internal ADR § 4 順守)。
