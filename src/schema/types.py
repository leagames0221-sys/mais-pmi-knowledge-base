"""T5 Object Type Pydantic schema (internal ADR 順守、 2026-05-14 起草)
...
# === ID prefix patterns (internal ADR 順守、 T1-T4 非衝突) ===
...
class Decision(BaseModel):
    """internal Decision extend、 internal ADR § 1。 PMI 案件内個別 decision の ADR-style 構造化。"""
...
class Pattern(BaseModel):
    """internal Pattern extend、 internal ADR § 2。 cross-case 抽出 PMI pattern。"""
...
class PMICase(BaseModel):
    """internal Incident extend、 internal ADR § 3。 PMI 案件 lifecycle 全段階 root entity。"""
...
class Outcome(BaseModel):
    """internal Pattern overlay extend、 internal ADR § 4。 Decision 結果 measurable KPI delta + outcome_class。"""
...
class ReferencePaper(BaseModel):
    """internal FileResource extend、 internal ADR § 5。 paper RAG ingestion entity (PoC = 合成 paper)。"""
...
# === 6. AssistantQuery (LIL、 新規起草、 ★★★ 立証責任 internal ADR § 6) ===
...
class AssistantQuery(BaseModel):
    """新規起草 (★★★ 立証責任明記、 internal ADR § 6)。 doctrine: analogical-recall doctrine literal 実装基盤の query trace 専用 type。
