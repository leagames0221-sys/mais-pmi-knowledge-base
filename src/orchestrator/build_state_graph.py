"""LangGraph state graph orchestrator (internal ADR § 4 順守、 2026-05-14 起草)
...
internal ADR § 4 drift note: ADR description「7 node」 vs DAG diagram「9 stages」 = literal drift evidence。
本 実装 = literal diagram (9 stages) 順守、 doctrine: drift-prevention で logbook 記録 + Week 3 internal ADR で literal ADR 訂正候補。

9 node DAG (internal ADR § 4 diagram literal):
...
# ============================================================================
# Multi-hop traversal default (internal ADR § 4 + internal ontology 既存設定 inherit + doctrine: consumer-hw)
# ============================================================================
...
class OrchestratorState(TypedDict, total=False):
    """9 node state shape (internal ADR § 4 diagram 順守)。
...
# ============================================================================
# Node 2: extract_entities (graphrag_native + relationship 抽出 内蔵、 internal ADR § 4 diagram 順守)
# ============================================================================
...
def node_similar_case_rank(state: OrchestratorState) -> dict[str, Any]:
    """5 dim weighted similarity top-K=3 rank (internal ADR § 3 連携)。

    Week 2 PoC simplification: query PMICase は candidates[0] 仮置き
    (Week 3 internal ADR で literal query 構造化 強化、 audit log に「stub」 明示)。
    """
    ...
    if not candidates:
        audit = _append_audit(
            state,
            {
                ...
                "top_k": 0,
                "note": "no candidates provided (Week 3 internal ADR で query PMICase build 強化)",
            },
        )
...
# ============================================================================
# Node 8: assistant_recommend (Week 3 internal ADR § 1 active 化 supersede、 internal ADR update reflect)
# ============================================================================


def node_assistant_recommend(state: OrchestratorState) -> dict[str, Any]:
    """Assistant 型対話 full active (internal ADR § 1 + internal ADR update node 8 supersede)。
    ...
    audit = _append_audit(
        state,
        {
            ...
            "stub": False,  # Week 3 active (internal ADR update reflect、 internal ADR § 1)
            "user_role": user_role,
        },
    )
...
# legacy alias (doctrine: doc-sync、 internal ADR update「node_assistant_recommend_placeholder → node_assistant_recommend」 supersede 記録)
...
def node_audit_log_emit(state: OrchestratorState) -> dict[str, Any]:
    """AssistantQuery schema literal emit (audit trail full)。 Week 3 internal ADR で persistent storage active 化。"""
    query = state.get("query_text_redacted", "")
    top_k_cases = state.get("top_k_cases", [])
    pending_recs = state.get("pending_recommendations", [])
    user_role_raw = state.get("user_role", "junior_consultant")
    # LIL id は timestamp ms 末尾 6 桁 (Week 3 internal ADR で counter base + persistent storage active)
    ...
    assistant_query = AssistantQuery(
        ...
        retrieved_papers=[],  # Week 3 internal ADR で RAG paper ingestion active
...
# ============================================================================
# Conditional routing (internal ADR § 4)
# ============================================================================
...
# ============================================================================
# Build state graph (LangGraph compiled、 internal ADR § 4)
# ============================================================================
...
def build_state_graph(checkpoint: Any = None) -> Any:
    ...
    Raises:
        RuntimeError: langgraph not installed (internal ADR update pin 違反)
    """
    try:
        from langgraph.graph import END, StateGraph  # type: ignore[import-not-found]
    except ImportError as exc:
        raise RuntimeError(
            "langgraph not installed (internal ADR update audit fact、 internal ADR § 4)。 "
            "Run: pip install 'langgraph>=1.2.0,<2.0'"
        ) from exc

    builder = StateGraph(OrchestratorState)

    # 9 node 登録 (internal ADR § 4 diagram literal 順守)
    ...
    # internal ADR update reflect: assistant_recommend_placeholder → assistant_recommend (Week 3 active 化、 internal ADR § 1)
...
# ============================================================================
# Multi-hop traversal helper (internal ADR § 4 + internal ontology 既存設定 inherit)
# ============================================================================
...
def multi_hop_traverse(
    ...
    Args:
        ...
        max_hops: 最大 hop (default 3、 internal ADR § 4 doctrine: consumer-hw 順守)
...
__all__ = [
    ...
    "node_assistant_recommend_placeholder",  # legacy alias、 internal ADR update reflect
    "node_audit_log_emit",
    "route_after_extract",
    "route_after_ontology_gate",
    "route_after_pii_check",
